...
# Struktur: {student_id: {"name": string, "courses": {course: grade, ...}}}
grades = {
...
def get_grades(item):
    ...
    physics_grade = courses.get("physics", 0)  # 0 hvis ikke tar fysikk
    cpp_grade = courses.get("c++", 0)          # 0 hvis ikke tar c++
...
for student_id, info in sorted_dict.items():
    ...
    total = physics + cpp
    print(f"ID {student_id}: {name:20} - Fysikk: {physics}, C++: {cpp}, Total: {total}")
